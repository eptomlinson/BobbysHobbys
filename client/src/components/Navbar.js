    ...
    <div className="pos-f-t">
      ...
      <nav style={styles} className="navbar">
        <button style={buttonStyle} align="center" className="navbar-toggler" type="button" data-toggle="collapse" data-target="#navbarToggleExternalContent" aria-controls="navbarToggleExternalContent" aria-expanded="false" aria-label="Toggle navigation">
          <img style={styles2} className="bobbyImage" src={whiteorangetransparent} alt="bobbyorange"></img>
          <span className="navbar-toggler-icon"><div style={{ fontSize: 20 }}>^</div></span>
        </button>
        <div className="row">
          <Link to="/favorites"><button style={{fontFamily: `'Assistant', sans-serif`, color: "#BF3A0D", size: 25, marginRight: 5, border: "1px solid #BF3A0D" }} className="btn btn-outline-light btn-space" >Favorites</button></Link>
          <PostModal onChange={handleInputChange} submitPost={handleFormSubmit} />

          <Link to="/">
            <button onClick={handleClick} type="button" className="btn btn-outline-light btn-space" style={{ marginRight: 5, fontFamily: `'Assistant', sans-serif`}}>Logout</button>
          </Link>
        </div>
      </nav>

      <img style={{ height: 400 }} className="bobbyround" src={bobbyround2} alt="bobby round"></img>
